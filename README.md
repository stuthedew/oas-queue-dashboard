...
- **Root cause of** is how many items an item's `root-cause-of:` names, after its own id and any id no item in the store carries are dropped, mirroring docket's `model.root_cause_faults`. At `MIN_ROOT_CAUSE_ITEMS` (3) or more, docket reads the claim as a generator and the page marks it; below that it is an ordinary item. The count is resolved in `extract.py`, which is the only side holding every id in the store, and items naming an id that does not resolve are reported in the page's banner rather than quietly counted short.
